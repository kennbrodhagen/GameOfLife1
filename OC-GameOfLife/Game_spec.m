...
@interface Game_spec : SenTestCase
@end
...
-(void) test_Creation
{
	...
	STAssertNotNil(gameCreated, nil);
}
...
-(void) test_Counts_Living_Neighbors
{
	...
	STAssertEquals(1u, [_game livingCount:neighbors], nil);
...
-(void) test_Example_Game_Round
{
	...
	STAssertEquals([nextBoard cellAtRow:0 andColumn:0].state, CellStateAlive, nil);
	STAssertEquals([nextBoard cellAtRow:0 andColumn:1].state, CellStateAlive, nil);
	STAssertEquals([nextBoard cellAtRow:1 andColumn:1].state, CellStateAlive, nil);
	STAssertEquals([nextBoard cellAtRow:1 andColumn:0].state, CellStateAlive, nil);
}
